...
data "terraform_remote_state" "artifactory" {
  backend = "consul"
  environment = "${terraform.env}"
  config {
    path = "aws/pcs/artifactory/tfstate"
  }
}
...
data "consul_keys" "config" {
  ...
  key {
    name    = "hostname"
    path    = "aws/pcs/config/${var.org}/${var.environment}/artifactory/hostname"
    default = "${length(var.hostname) > 0 ? var.hostname : "artifactory" }"   
  }
  key {
    name    = "instance_type"
    path    = "aws/pcs/config/${var.org}/${var.environment}/artifactory/instance_type"
    default = "m4.large"
  }
  key {
    name    = "install_url"
